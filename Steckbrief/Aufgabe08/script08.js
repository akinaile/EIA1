...
function toggleThis(OneHTMLElement, TwoHTMLElement) {
    OneHTMLElement.classList.add("isHidden");
    TwoHTMLElement.classList.remove("isHidden");
}
...
playB.addEventListener("click", function () {
    toggleThis(this, stopB);
    boolPlayStop = true;
    triggerSound();
});
stopB.addEventListener("click", function () {
    toggleThis(this, playB);
    boolPlayStop = false;
    triggerSound();
});
...
function triggerSound() {
    if (let)
        i: number = 0;
    i < intervallArray.length;
    i++;
    {
        intervallArray = setInterval(function () {
